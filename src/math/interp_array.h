...
struct InterpArray {
  ...
  T &set(const unsigned int index) { return data[index]; }
  void set(const unsigned int index, T val) { data[index] = val; }
  const T operator[](const unsigned int index) const { return data[index]; }
  const T get(const unsigned int index) const { return data[index]; }

  // Return an interpolated value where index = 0 and index = Size are the first
  // element
  ...
  // Return an interpolated value. index = 0 returns the first element and index
  // = Size-1 returns the last element Boundary checking is not done Todo: check
  // if the unsigned int cast uses the native ARM float32->int32 conversion. If
  // not, figure out how to do that while keeping portability
  ...
  // Return an interpolated value where phase = 0 and phase = N (any integer)
