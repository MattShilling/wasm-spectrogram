...
template <int UpdateRateHz> struct TriangleOscillator {
  ...
  // Returns 0 .. 1
  float process_float() { return static_cast<float>(process()) / 4294967295.f; }
...
template <int UpdateRateHz> struct PhaseAccum {
  PhaseAccum(uint32_t freq) : increment_(freq * freq_units) {}
  ...
  uint32_t Process() { return process(); }
  float process_float() {
...
private:
  static constexpr uint32_t max_ = 0xFFFFFFFF;
  static constexpr float _to_float_convert =
